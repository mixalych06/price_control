...
from keyboards.kb_all import gen_markup_sellers, gen_seller_markup_, gen__markup_pagination, gen_markup_del_sellers
...
async def start_suadmin(message: types.Message):
    ...
    if message.from_user.id not in db.bd_get_su_admins():
        db.bd_add_su_admin(message.from_user.id, message.from_user.first_name)
...
async def shows_sellers(message: types.Message):
    ...
    if sellers:
        await message.answer(text='⬇️<b>Выбери продавца</b>⬇️',
                             reply_markup=await gen_markup_sellers(sellers, 'moreSeller'))
    else:
...
async def show_seller(cq: types.CallbackQuery):
    ...
    await cq.message.edit_text(text=f'<b>Продавец:</b> {name_seller[0]}\n'
                                    f'<b>ID:</b> {name_seller[1]}\n'
    ...
    if len(products):
        try:
            await cq.message.edit_text(text=f'<b>Продавец:</b> {products[page][0]}\n'
                                            ...
                                            f'<b>Цена:</b> {products[page][3]} руб.\n'
                                            f'<b>Мin цена:</b> {products[page][4]} руб.',
                                       reply_markup=await gen__markup_pagination(page, inl_com[1], int(inl_com[2]),
                                                                                 len(products), products[page][1]))
        except:
            pass
    else:
...
async def shows_users(message: types.Message):
    ...
    if 'Администраторы' in message.text:
        ...
        if admins:
            for admin in admins:
                await message.answer(text=f'<b>Администратор {admin[1]}</b>\nID: {admin[0]}', reply_markup=InlineKeyboardMarkup(row_width=2).add(
                    InlineKeyboardButton(text='Удалить', callback_data=f'del_admin:{admin[0]}'),
                    InlineKeyboardButton(text='Уменьшить права', callback_data=f'null')))
        else:
        ...
        if users:
            for user in users:
                await message.answer(text=f'<b>Пользователь {user[1]}</b>\nID: {user[0]}', reply_markup=InlineKeyboardMarkup(row_width=2).add(
                    InlineKeyboardButton(text='Удалить', callback_data=f'del_user:{user[0]}'),
                    InlineKeyboardButton(text='Расширить права', callback_data=f'null')))
        else:
...
async def enable_tracking(cq: types.CallbackQuery, state: FSMContext):
    '''Прекращает отслеживать мин цену'''
    await cq.message.edit_text('Введите цену товара.',
...
async def add_min_prise(message: types.Message, state: FSMContext):
    async with state.proxy() as data:
        ...
        try:
            ...
            await message.answer(text=f'"{message.text}"Не верное значение. Введите цену в рублях цифрами или нажмите отмена')
            await message.delete()
