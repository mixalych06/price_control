...
from keyboards.kb_users import keyboard_user_main, gen__markup_pagination_for_user,\
    gen_seller_markup_user, gen_markup_sellers_user
...
async def start_user(message: types.Message):
    await message.reply('Привет, admin', reply_markup=keyboard_user_main)
...
async def shows_sellers_user(message: types.Message):
    ...
    if sellers:
        await message.answer(text='👇<b>Выберите продавца</b>👇', reply_markup=await gen_markup_sellers_user(sellers))
    else:
